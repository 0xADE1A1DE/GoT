...
#define DEFAULT_NUMBER_OF_TESTS (100000)
...
ComparisonResult test_gate(Gate *gate, uintptr_t gate_io_memory, int test_index);
ComparisonResult compare_to_simulation(Gate *gate, GateResults &results);
...
int test_gates(int argc, char *argv[]) {
    ...
    int i = 0;
    int number_of_gates = number_of_classic_bt_gates + number_of_counter_bt_gates;
    Gate *gates_to_test[number_of_gates];
    for (Gate *current : classic_bt_gates_to_test.gates)
    ...
    for (int gate_index = 0; gate_index < number_of_gates; gate_index++) {
        for (int i = 0; i < number_of_tests_per_gate; i++) {
            ...
            ComparisonResult result = test_gate(current_gate, gate_io_memory, i);
...
ComparisonResult test_gate(Gate *gate, uintptr_t gate_io_memory, int test_index) {
    ...
    ComparisonResult res = compare_to_simulation(gate, results);
...
ComparisonResult compare_to_simulation(Gate *gate, GateResults &results) {
    ...
    return (ComparisonResult) { .first_simulated_output_state = simulation.outputs[0], .as_expected = (count == gate->m_fan_out), .count = count };
}
