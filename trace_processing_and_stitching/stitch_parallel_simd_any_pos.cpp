...
#define VECTOR_SIZE 32
    ...
    {
      currentMajorityVector[vec_ctr] = (majorityMap[i][1] > majorityMap[i][0] ? 'm' : 'S');
    }
    ...
    for (int i = 0; i < numberOfVectors; i++) {
      currentMajorityVectors[i] = _mm256_loadu_si256((__m256i_u *)((uintptr_t)currentMajorityVector.data() + i * VECTOR_SIZE));
    }
    ...
    for (intptr_t i = thread_start_index; i < thread_end_index; i++)
    {
      ...
      currentKeyvector.resize(3 * vectorSize + chunk.key.length());
      ...
      for (int64_t start_index = whole_start - chunk.key.length(), key_offset = chunk.key.length();
...

{
      ...
      else
      {
        ...
        int64_t initialS_end_index = -1;
        double initialSWeight = 1.0;
        
        for (int i = 0; i < (int64_t)max_matches_chunk.key.length(); i++) {
          if (max_matches_chunk.key[i] == 'm')
            break;
          initialS_end_index = i;
        }
        
        if (initialS_end_index >= 0)
        initialSWeight = 1.0 / (double)initialS_end_index;
             ...
             index < (int64_t)max_matches_chunk.key.length(); index++, map_index++)
        {
          ...
          if (index <= initialS_end_index) {
            
	    
            majorityMap[map_index][0] += 0.25;
          }
          else if (max_matches_chunk.key[index] == 'S') {
