...
struct TreeAmplificationParamaters {
...
private:
    ...
    // Todo: refactor these?
    // Rename cache_lines_per_layer to indices_per_layer?
    int get_layer_start(int depth);
    int cache_lines_per_layer(int depth);

    void prefetch(uintptr_t speculatee, int tree_index, int meta_depth, uint64_t &trash);
    uintptr_t get_prefetch_speculatee_address(uintptr_t addr);
    uintptr_t access_pattern(int tree_index, int depth, int meta_depth, int layer_index);

    int leaf_to_tree_index(int leaf_index, int meta_depth);
    void tree_amplify(uintptr_t tree_root, int tree_index, int meta_depth, uint64_t &trash);
    void meta_amplify(uintptr_t speculatee, int leaf_index, int meta_depth, int leafs_to_recurse, uint64_t &trash);
};

extern Amplification &ta_small;
extern Amplification &ta_100us;
extern Amplification &ta_1ms;
extern Amplification &ta_10ms;
