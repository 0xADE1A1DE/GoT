    ...
    for it in tqdm.tqdm(range(args.iterations)):
        for page in tqdm.tqdm(page_amounts, leave=False):
            for per_ev in tqdm.tqdm(range(1,5), leave=False):
                for amplification_number in tqdm.tqdm(range(5, 55, 5), leave=False):
                    if page / per_ev > 100:
                        continue
                    key = f"{page * 7 / shrink_factor}, {per_ev}, {amplification_number}"

                    if key not in box_plot_dict:
                        box_plot_dict[key] = []

                    with open(f"/tmp/retention_results/{page}_{per_ev}.txt", "a") as f:
                        res = binary("test_retention", "-p", str(page), "-e", str(per_ev), "-n", str(amplification_number)).stdout.decode("ascii")
