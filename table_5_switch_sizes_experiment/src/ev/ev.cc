...
using ChosenProbeParamters = ProbeParamters<7, measure::Measure<measure::t_RDTSC>>;

EvictionSetManager::EvictionSetManager(size_t allocation_size) {
    initialize_allocation((void **)&eviction_set_memory, allocation_size);
}
...
EvictionSet *EvictionSetManager::reduce_eviction_set(uintptr_t target, int retries) {
    ...
    for (int retry_index = 0; retry_index < retries; retry_index++) {
        ...
        backtrack_count = 0;
        typename ChosenProbeParamters::MeasureClass measurer;
        if (reduce_eviction_set<ChosenProbeParamters>(sentinel, INITIAL_SAMPLE_SIZE, target, measurer)) {
...
template<typename probe_paramaters>
uint64_t __always_inline probe_once(node *set, uintptr_t candidate, typename probe_paramaters::MeasureClass m) {
    uint64_t trash = 0;
...
template<typename probe_paramaters>
bool __always_inline probe(node *set, uintptr_t candidate, typename probe_paramaters::MeasureClass m) {
    uint32_t total = 0;
    for (int i = 0; i < probe_paramaters::probe_trials; i++) {
        uint64_t result = probe_once<probe_paramaters>(set, candidate, m);
        total += !m.in_cache(result);
    }
    return total > (probe_paramaters::probe_trials / 2);
}

template<typename probe_paramaters>
bool EvictionSetManager::reduce_eviction_set(node *sentinel, uint32_t length, uintptr_t evictee, typename probe_paramaters::MeasureClass m) {
    ...
    for (uint32_t i = 0; i < N; i++) {
        ...
        if (probe<probe_paramaters>(sentinel->next, evictee, m)) {
#ifdef REDUCE_DBG
            printf("Going in with %d at idx %d\n", length - advance, i);
#endif
            if (reduce_eviction_set<probe_paramaters>(sentinel, length - advance, evictee, m))
                return true;
