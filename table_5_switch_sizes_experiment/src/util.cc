...
const int cache_sets = LLC_CACHE_SIZE / CACHE_LINE_SIZE / LLC_CACHE_ASSOCIATIVITY / CACHE_SLICES;
...
void fetch_address(uintptr_t address, address_state to_state) {
    constexpr auto l1_eviction_set = non_cow_memory<PAGE_SIZE_ * L1_CACHE_ASSOCIATIVITY>();
    switch (to_state) {
    case L1:
...
uintptr_t mmap_symbol_from_binary(char *path, char *symbol) {
    ...
    uintptr_t mmap_result = (uintptr_t)mmap(0, PAGE_SIZE_, PROT_READ, MAP_PRIVATE, fd, file_offset & ~(PAGE_SIZE_ - 1));
    close(fd);
